...
// Bull Flag (LONG) — tighter: require vol_ratio > 1.2
function detectBullFlag(c,i,rsi,vr,ma20){
  ...
  let conf=1;
  if(vr>1.2)conf++;          // TIGHTENED: was 1.0
...
// Double Top (SHORT) — tighter: RSI cap at 85, vol_ratio > 1.2
function detectDoubleTop(c,i,rsi,vr,ma20){
  ...
  // TIGHTENED: RSI 65-85 (was just >65). Extreme RSI >85 = blowoff top, too risky
  if(rsi<65||rsi>85)return null;
  if(c0<ma20*1.02)return null;
  let conf=1;
  if(rsi>70&&rsi<82)conf++;   // TIGHTENED: sweet spot, not extreme
  if(vr>1.2)conf++;            // TIGHTENED: was 1.0
...
async function run(){
  console.log('PERPS ALCHEMY — R11-TIGHT (Reaper v1.0 + tighter filters)');
  console.log('Config: dynamic 2% stop, 2x target, 15d max hold, vol>1.2, RSI 65-85\n');
