...
async function run(){
  console.log('PERPS ALCHEMY — R11-V2');
  console.log('Config: dynamic 2% stop, 2x target, 15d hold, vol>1.2, RSI 65-82');
  console.log('Regime: suppress Double Top shorts when SMA20>SMA50 by >5% (very strong bull only)\n');
  ...
  for(const coin of COINS){
    ...
    for(let i=20;i<c.length-1;i++){
      ...
      // Regime filter: only suppress shorts in VERY strong bull (>5%)
      if(s.side==='short'){
      ...
      // No bear filter for longs — Bull Flag is already a reversal pattern that works in downtrends
